...
OpenClaw uses a worktree-first execution model. The PREQ CLI/dispatcher prepares an isolated workspace, writes the PREQ prompt to disk, then launches the requested coding agent with a short bootstrap instruction.

The preferred public package and command are `@sonim1/preqstation` and `preqstation`. Some current plugin and config identifiers still use `preqstation-dispatcher` or `preqstation-openclaw` for compatibility.
...
<Aside>
Use the PREQ CLI docs for user-facing install and update commands. Treat `preqstation-dispatcher` as a legacy/internal identifier when it appears in plugin or config output.
</Aside>
