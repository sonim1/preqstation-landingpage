...
5. Open `/connections` to review the install. Revoke stale or expired clients there, then reconnect.

6. If the client still looks stuck, remove and re-add the server:
```bash
...
### OAuth client is visible but no longer usable

**Symptom:** The connection exists in `/connections`, but calls fail after the expiry or revocation timestamp.

**Fix:** Reconnect the client through the OAuth flow. `/connections` is the page to inspect engine, status, last used time, and expiry before reauthorizing.
...
**Symptom:** `preq_list_tasks` or other tools fail with `401 Unauthorized` in shell-helper mode or the legacy local bridge
...
- Go to the PREQSTATION web app → the legacy API Keys view
...
**Symptom:** Token auth fails even with correct-looking token in shell-helper mode or the legacy local bridge. This does not apply to OAuth-backed `/mcp` installs.

<Tabs>
<TabItem label="Cause: Expired Token">
**Check:** In the PREQSTATION web app, go to the legacy API Keys view and check expiration date.
...
**Check:** List tokens in the legacy API Keys view to see if token is still there.
