---
title: Task Lifecycle
description: Task status workflow, run-state overlay, and lifecycle actions.
---

import { Aside } from '@astrojs/starlight/components';
import Mermaid from '../../../components/Mermaid.astro';

Tasks in PreqStation have two dimensions:

- **Workflow status** — where the card sits in the lifecycle
- **Run state** — whether an agent has requested or claimed execution right now

That split is the main difference from the older `in_progress` / `review` model.

The [Work Graph](/guide/web-app/work-graph/) is attached to the same task but records a different layer: the agent's execution path, evidence, decisions, and workflow memory. Lifecycle tools move the card through review states. Graph tools record how the work was performed.

## UI, API, and Agent Actions

| User sees | API value | Common action |
|---|---|---|
| Inbox column | `inbox` | create a task, then let an agent plan it |
| Todo column | `todo` | dispatch or start implementation |
| Hold column | `hold` | fix the blocker or add missing context |
| Ready column | `ready` | review result, run QA, then decide whether it is done |
| Done column | `done` | archive or reopen only if follow-up work appears |
| Queued badge | `run_state=queued` | wait for the dispatcher or inspect Hermes/Telegram |
| Working badge | payload `run_state=working` | wait for the worker or inspect operator-host logs |

## Workflow Model

<Mermaid
  chart={`
flowchart LR
  inbox[Inbox] --> todo[Todo]
  todo --> ready[Ready]
  ready --> done[Done]
  todo --> hold[Hold]
  hold --> ready
  queued[Queued overlay] -.-> todo
  running[Running overlay] -.-> ready
`}
  caption="The happy path is inbox -> todo -> ready -> done. Hold is a side branch for blocked or paused work."
/>

## Workflow Statuses

| Status | Meaning | Typical next action |
|---|---|---|
| **Inbox** | New task, awaiting planning | Agent claims the task, reads local code, then runs `preq_plan_task()` |
| **Todo** | Planned and ready for implementation | Agent claims the task with `preq_start_task()` and begins work |
| **Hold** | Blocked or paused | Agent retries after resolving the blocker, or stays on hold |
| **Ready** | Implementation is complete and waiting for verification | Agent or owner runs verification and then `preq_review_task()` |
| **Done** | Verified and complete | Finished unless reopened manually |
| **Archived** | Hidden from active board views | Manual archival only |

## Run State Overlay

Run state is tracked independently from workflow status:

| Run State | Meaning |
|---|---|
| `null` | No active execution claim |
| `queued` | Requested or dispatched, but not yet claimed by an agent |
| `running` internally, `working` in PREQ/MCP payloads | Claimed by an agent through `preq_start_task()` |

Telegram dispatch can mark a task as `queued` before an agent picks it up. `preq_start_task()` upgrades that to the active working state.

The examples below use MCP tool names because this is an API lifecycle reference. Detached dispatch workers receive equivalent `preqstation task ...` commands in `.preqstation-instructions.txt`.

Graph-aware workers should also record their execution with `preqstation-agent graph ...` or the MCP graph tools. `metadata.workflow_profile` belongs on graph node metadata, not on the task workflow status itself.

## Lifecycle Branches

Agents fetch the task once, resolve the current workflow status once, then execute exactly one matching branch.

### From Inbox

```text
preq_get_task(PROJ-284)         → status=inbox
preq_start_task(PROJ-284)       → run_state=working
read local code + write plan
preq_plan_task(PROJ-284, ...)   → status=todo, run_state=null
stop
```

### From Todo

```text
preq_get_task(PROJ-284)         → status=todo
preq_start_task(PROJ-284)       → run_state=working
implement + test + deploy
preq_complete_task(PROJ-284, ...) → status=ready, run_state=null
stop
```

### From Hold

```text
preq_get_task(PROJ-284)         → status=hold
preq_start_task(PROJ-284)       → run_state=working
investigate blocker / resume work
if resolved: preq_complete_task(PROJ-284, ...) → ready
if still blocked: preq_block_task(PROJ-284, ...) → hold
stop
```

### From Ready

```text
preq_get_task(PROJ-284)         → status=ready
preq_start_task(PROJ-284)       → run_state=working
run verification (tests, build, lint)
if passing: preq_review_task(PROJ-284) → done
if failing: preq_block_task(PROJ-284, ...) → hold
stop
```

<Aside>
`preq_start_task()` is an execution-claim action. It does not create an `in_progress` workflow column anymore.
</Aside>

## Workflow Transitions

| From | To | Trigger | Command |
|---|---|---|---|
| `inbox` | `todo` | Agent plan is uploaded | `preq_plan_task()` |
| `todo` | `ready` | Implementation is complete | `preq_complete_task()` |
| `hold` | `ready` | Blocker is resolved and work is complete | `preq_complete_task()` |
| `ready` | `done` | Verification succeeds | `preq_review_task()` |
| `inbox` | `hold` | Planning is blocked | `preq_block_task()` |
| `todo` | `hold` | Implementation is blocked | `preq_block_task()` |
| `hold` | `hold` | Blocker remains | `preq_block_task()` |
| `ready` | `hold` | Verification fails | `preq_block_task()` |

`preq_update_task_status()` is the manual escape hatch for direct status changes such as reopening or archiving a card.

## Example Timeline

```text
09:00 user creates PROJ-284 in Inbox
09:02 agent runs preq_plan_task -> Todo
09:05 user dispatches with H Telegram -> run_state=queued
09:06 worker runs preq_start_task -> run_state=working
09:20 worker runs preq_complete_task -> Ready
09:25 owner verifies and runs preq_review_task -> Done
```

The workflow column tells you the review state. The run-state overlay tells you whether an execution process is currently waiting or active.

## Result Payload

When an agent submits `preq_complete_task()`, it uploads a result object like:

```json
{
  "summary": "Implemented JWT authentication. All tests passing.",
  "tests": "npm test",
  "pr_url": "https://github.com/project/pull/456",
  "notes": "Stored JWT in an httpOnly cookie.",
  "artifacts": [
    {
      "type": "image",
      "title": "Verification screenshot",
      "url": "https://fast.io/s/example",
      "provider": "fastio",
      "access": "quickshare",
      "expires": "2026-05-10T00:00:00.000Z"
    }
  ],
  "completed_at": "2026-03-12T14:22:00.000Z"
}
```

PreqStation stores the result summary as a work log entry attached to the task. Structured `artifacts` are stored separately from markdown note/report content, so agents should not append `Artifacts:` blocks to task notes.

## Deployment Strategy

Task execution still respects the project deployment strategy:

| Strategy | Behavior |
|---|---|
| `direct_commit` | Merge and push to the default branch. |
| `feature_branch` | Push the task branch and optionally create a PR. |

Missing, invalid, and legacy `none` values are normalized to `direct_commit`.

Agents resolve the strategy through `preq_get_project_settings()` before deploy steps.

<Aside>
Because workflow status and run state are separate, you can reopen a task with the Kanban board or `/tasks/:id/status` without inventing a fake "in progress" workflow column.
</Aside>
