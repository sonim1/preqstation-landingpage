...
`dispatch_target` is returned in task payloads but is not a normal agent-written field. The web app currently manages it through dispatch actions, with values `claude-code-channel`, `telegram`, or `null`.
...
| Method | Endpoint | Purpose |
|---|---|---|
| `POST` | `/dispatch/claude-code` | Queue `dispatch_target=claude-code-channel` |
...
| `POST` | `/dispatch/claude-code` | Queue Claude Code dispatch |
