---
title: Deploy Strategy
description: Deployment strategy contract for PreqStation task execution
---

import { Aside, Tabs, TabItem } from '@astrojs/starlight/components';

Before executing any git operations on a task, agents must resolve and follow the project's deployment strategy. This contract defines how code changes should be committed, pushed, and deployed.

## Choose a Strategy

| You want | Use | Result |
|---|---|---|
| fastest single-owner flow | `direct_commit` | verified work lands on the default branch |
| review before merge | `feature_branch` with `auto_pr=true` | task branch is pushed and a PR is opened |
| manual PR creation | `feature_branch` with `auto_pr=false` | branch is pushed and the result reports what to review |

Changing strategy affects where code is pushed, not how task lifecycle status works.

## Strategy Types

Each project configures one of two active strategies:

| Strategy | Behavior | When to Use |
|----------|----------|-------------|
| `direct_commit` | Merge worktree commits to default branch and push | Single-branch production deployments |
| `feature_branch` | Push worktree branch; create PR if auto_pr=true | Multi-environment, review-based deployments |

Missing, invalid, and legacy `none` values are normalized to `direct_commit`.

## Configuration

### Resolving Strategy

Before any git action, resolve the deployment strategy:

1. Run `preqstation task get <task_id>`
2. Check `task.deploy_strategy` in the response
3. If missing, run `preqstation project settings --project <project_key>`
4. Use settings: `deploy_strategy`, `deploy_default_branch`, `deploy_auto_pr`, `deploy_commit_on_review`, `deploy_squash_merge`

### Default Values

When strategy is missing or invalid:

```json
{
  "strategy": "direct_commit",
  "default_branch": "main",
  "auto_pr": false,
  "commit_on_review": true,
  "squash_merge": true
}
```

### Settings Table

| Setting | Type | Purpose |
|---------|------|---------|
| `deploy_strategy` | string | `direct_commit` or `feature_branch` |
| `deploy_default_branch` | string | Target branch name (usually `main`) |
| `deploy_auto_pr` | boolean | Auto-create PR (feature_branch only) |
| `deploy_commit_on_review` | boolean | Require remote push before the task can move to `ready` |
| `deploy_squash_merge` | boolean | Squash commits (direct_commit only) |

## Strategy: direct_commit

**Commit and push to default branch.** No PR created.

Use this for:
- Single-branch deployments
- CI/CD auto-deployment from main
- Trust-based team workflows

### Execution Steps

1. Work in isolated worktree
2. Make commits and run tests
3. **Before `preqstation task complete`:**

Create a `result.json` payload before completion. Include at least `summary`, and include `tests`, `branchName`, and `prUrl` when they apply:

```json
{
  "summary": "Implemented rate limiting",
  "tests": "npm run test",
  "branchName": "preqstation/PROJ/rate-limit",
  "prUrl": "https://github.com/org/repo/pull/123"
}
```

<Tabs>
<TabItem label="Squash Merge (default)">
```bash
# In primary checkout: update and merge with squash
git -C <project_cwd> checkout <default_branch>
git -C <project_cwd> pull origin <default_branch>
git -C <project_cwd> merge --squash <worktree_branch>
git -C <project_cwd> commit -m "PROJ-284: Implement rate limiting"
git -C <project_cwd> push origin <default_branch>

# Verify push succeeded before completing, then write result.json and complete.
git ls-remote --heads origin <default_branch> | grep -q .
preqstation task complete PROJ-284 --json-file result.json
```
</TabItem>
<TabItem label="Regular Merge">
When `deploy_squash_merge=false`:

```bash
git -C <project_cwd> checkout <default_branch>
git -C <project_cwd> pull origin <default_branch>
git -C <project_cwd> merge <worktree_branch>
git -C <project_cwd> push origin <default_branch>
```
</TabItem>
</Tabs>

## Strategy: feature_branch

**Push worktree branch; optionally create PR.** Multi-environment friendly.

Use this for:
- Pull-request-based workflows
- Feature review and approval
- Multi-environment deployments (dev → staging → prod)

### Execution Steps

1. Work in isolated worktree (branch: `preqstation/<project_key>/<feature>`)
2. Make commits and run tests
3. **Before `preqstation task complete`:**

```bash
# Push feature branch
git -C <project_cwd> push origin <worktree_branch>

# Verify push succeeded
git ls-remote --heads origin <worktree_branch> | grep -q . || \
  { printf '%s\n' "Failed to push feature branch" > blocker.md; preqstation task block PROJ-284 --reason-file blocker.md; exit 1; }

# If auto_pr=true, create PR (requires gh CLI or an authenticated GitHub integration)
if [ "$deploy_auto_pr" = "true" ]; then
  gh pr create --head <worktree_branch> --base <default_branch> \
    --title "PROJ-284: Implement rate limiting" \
    --body "Task: PROJ-284"
fi

# Complete task with summary, branch, tests, and PR URL in result.json
preqstation task complete PROJ-284 --json-file result.json
```

## commit_on_review Rule

When `deploy_commit_on_review=true` (default):

- Agents must verify the remote push before moving the task to `ready`
- Check branch exists on origin before calling `preqstation task complete`
- Do not call `preqstation task complete` without a successful push

**Example verification:**
```bash
if ! git ls-remote --heads origin "$branch_name" | grep -q .; then
  printf '%s\n' "Remote push verification failed" > blocker.md
  preqstation task block PROJ-284 --reason-file blocker.md
  exit 1
fi

preqstation task complete PROJ-284 --json-file result.json
```

When `deploy_commit_on_review=false`:

- Moving to `ready` is allowed without mandatory remote push
- Useful for draft/WIP workflows

## Complete Execution Flow

```
1. Fetch task: `preqstation task get PROJ-284`
2. Fetch strategy: `preqstation project settings --project PROJ`
3. Resolve strategy from response
4. Execute code changes in worktree
5. Deploy per strategy:
   - direct_commit: merge + push default_branch
   - feature_branch: push <worktree_branch>, create PR if auto_pr=true
6. Verify remote push (if commit_on_review=true)
7. Call `preqstation task complete PROJ-284 --json-file result.json` with summary, branch, and PR URL
8. Clean up worktree: git worktree remove <cwd> --force
```

## Worktree Cleanup

Always clean up the worktree as the final step:

```bash
git -C <project_cwd> worktree remove <cwd> --force
git -C <project_cwd> worktree prune
```

This ensures isolation and prevents stale worktrees from accumulating.

<Aside>
Worktree cleanup is safe and should happen regardless of success or failure. It is part of the execution contract.
</Aside>
