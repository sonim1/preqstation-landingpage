...
| `deploy_commit_on_review` | boolean | Require remote push before review transition |
...
**Implementation:**
```bash
# Just update the task status without any git push
preq_complete_task PROJ-284 "Implementation complete" claude
```
...
git ls-remote --heads origin <default_branch> | grep -q . && \
  preq_complete_task PROJ-284 "Implemented rate limiting" claude
```
</TabItem>
...
git ls-remote --heads origin <worktree_branch> | grep -q . || \
  { preq_block_task PROJ-284 "Failed to push feature branch" claude; exit 1; }
...
- Agents must verify the remote push before moving to review
- Check branch exists on origin before calling `preq_complete_task`
- Do not transition to review without successful push

**Example verification:**
```bash
if ! git ls-remote --heads origin "$branch_name" | grep -q .; then
  preq_block_task PROJ-284 "Remote push verification failed" claude
  exit 1
fi

preq_complete_task PROJ-284 "..." claude
```

When `deploy_commit_on_review=false`:

- Review transition is allowed without mandatory remote push
