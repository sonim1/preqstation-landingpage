---
title: Deprecated Shell Helper Compatibility
description: Deprecated shell wrappers for PreqStation when MCP is unavailable.
---

import { Aside, Tabs, TabItem } from '@astrojs/starlight/components';

When MCP is unavailable, source `scripts/preqstation-api.sh` and use the deprecated shell wrappers directly. Prefer the remote PREQ `/mcp` endpoint with OAuth whenever your agent supports it.

<Aside type="caution">
Most new installs should not need this page. Use shell helpers only for compatibility clients, direct REST automation, or temporary debugging when remote MCP is unavailable.
</Aside>

## Return to Remote MCP When Possible

If your runtime now supports remote HTTP MCP, move back to the OAuth path:
...
After that, remove `PREQSTATION_TOKEN` from shell startup files unless another script still needs it.
