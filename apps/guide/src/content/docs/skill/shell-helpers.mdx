...
When MCP is unavailable, source `scripts/preqstation-api.sh` and use the shell wrappers directly. Prefer the remote PREQ `/mcp` endpoint with OAuth whenever your agent supports it.
