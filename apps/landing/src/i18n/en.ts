export default {
  ...
  hero: {
    ...
    description: "PreqStation exists for developers who cannot stay at the keyboard all day but still need work to keep moving. Prepare the task in the control plane, use the PREQ CLI to ready the local operator host, dispatch from the web app, integrations, or direct CLI when that fits, and come back to reviewable proof from Claude, Codex, or Gemini.",
  ...
  solution: {
    ...
    proof: {
      ...
      taskSummary: "PreqStation keeps the task scoped, assigned, and reviewable while the PREQ CLI prepares the local host that will run or dispatch the worker.",
      criteriaLabel: "Acceptance criteria",
      criteria: [
        "Queue and prepare the task in PreqStation",
        "Install and check the operator host with the PREQ CLI",
        "Dispatch through the web app, integrations, or direct CLI",
      ],
      meta: [
        { label: "Assigned engine", value: "Codex" },
        { label: "Operator check", value: "preqstation doctor" },
        { label: "Review gate", value: "notes + tests + branch/PR" },
      ],
      evidenceTitle: "Review evidence",
      evidence: [
        { label: "Branch", value: "task/pqsl-11/..." },
        { label: "Output", value: "Implementation notes and tests returned to PREQ" },
        { label: "Next", value: "Review evidence decides merge, deploy, or follow-up" },
      ],
    },
    ...
    aiItems: [
      ...
      { title: "Run on the prepared host", desc: "Use mapped projects, isolated worktrees, and the selected Claude, Codex, or Gemini runtime" },
  ...
  pipeline: {
    ...
    subtitle: "Prepare the task, ready the operator host, dispatch the worker, then review evidence before Done.",
    targetBadge: "TARGET: DIRECT EXECUTIONS",
    steps: [
      { id: "inbox", label: "Inbox", icon: "\ud83d\udce5", title: "Queue work", desc: "Capture the scope, acceptance criteria, and selected worker engine." },
      { id: "todo", label: "Todo", icon: "\ud83d\uddc2\ufe0f", title: "Prepare host", desc: "Run the PREQ CLI installer, doctor checks, and project mapping." },
      { id: "hold", label: "Hold", icon: "\u23f8\ufe0f", title: "Resolve blocks", desc: "Pause when mapping, runtime, or review input is missing." },
      { id: "ready", label: "Ready", icon: "\ud83d\udc40", title: "Review evidence", desc: "Check notes, tests, branch, or PR output before merge." },
      { id: "done", label: "Done", icon: "\u2705", title: "Ship result", desc: "Mark Done only after the reviewed result is accepted." },
    ],
  ...
  multiEngine: {
    ...
    subtitle: "Direct CLI, web, OpenClaw, Hermes, and Telegram dispatch choose a worker engine separately from the dispatch path",
    engines: [
      {
        accent: "bg-[#C15F3C]", logo: "C", logoBg: "bg-[#C15F3C]",
        name: "Claude Code", company: "Anthropic",
        desc: "Best when you want a strong general-purpose engine for complex changes, review, and local worker execution.",
        bestFor: ["General tasks", "Complex refactoring", "Review"],
      },
      {
        accent: "bg-[#00A67E]", logo: "Cx", logoBg: "bg-[#00A67E]",
        name: "Codex CLI", company: "OpenAI",
        desc: "Works as a PREQ worker for implementation, review, repeatable task execution, and direct CLI runs from a mapped operator host.",
        bestFor: ["Implementation", "Code review", "Direct CLI"],
      },
      {
        accent: "bg-[#078EFA]", logo: "G", logoBg: "bg-[#078EFA]",
        name: "Gemini CLI", company: "Google",
        desc: "Works as a PREQ worker for UI, docs, and larger-context tasks when the operator host has the runtime installed.",
        bestFor: ["UI/UX design", "Documentation", "Large context"],
      },
    ],
  },
  whoIsItFor: {
    ...
    forYou: {
      ...
      items: [
        { icon: "📱", title: "Ideas on the go", desc: "Capture scoped tasks when they appear and dispatch them when the host is ready." },
        { icon: "⚡", title: "Multi-project operator", desc: "Map several repos and keep project context attached to each PREQ task." },
        { icon: "🧠", title: "Context switcher", desc: "Return to notes, run state, and branch evidence instead of reconstructing a chat session." },
        { icon: "🤖", title: "Multi-engine strategist", desc: "Pick Claude, Codex, or Gemini per task after choosing the dispatch path." },
        { icon: "✅", title: "Quality-focused dev", desc: "Review notes, tests, branches, and PRs before marking work done." },
      ],
    },
    notForYou: {
      title: "Choose the right setup path",
      items: [
        { icon: "🧭", title: "First-time local operator", desc: "Start with Quick Start, install the PREQ CLI, run doctor checks, and map your project." },
        { icon: "🏗️", title: "Self-hosting the web app", desc: "Use Web App Setup for migrations, owner login, and the task control plane before wiring dispatch." },
        { icon: "🛠️", title: "Worker-only fallback", desc: "Use manual MCP or worker setup only when the CLI cannot register your runtime or you want direct worker access." },
        { icon: "📡", title: "Advanced dispatcher host", desc: "Add OpenClaw, Hermes, or Telegram when a prepared host should receive remote dispatches." },
      ],
    },
  },
  pricing: {
  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        answer: "Choose Claude Code, Codex CLI, or Gemini CLI as the worker engine. Direct CLI, web, OpenClaw, Hermes, and Telegram are dispatch paths, not engine limits.",
      },
      {
  ...
  cta: {
    ...
    subtitle: "Use Quick Start to prepare the operator host, map a project, dispatch a worker, and review the returned evidence in PreqStation.",
    proof: {
      eyebrow: "Current setup proof",
      title: "Install, check, map, dispatch, review",
      checklistLabel: "Operator-host checklist",
      checklist: [
        "npx -y @sonim1/preqstation@latest install",
        "preqstation doctor",
        "preqstation setup auto",
        "Dispatch via web app, integration, or direct CLI",
